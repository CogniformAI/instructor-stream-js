...
export const program = Micro.gen(function* () {
  ...
  return (
    n > 0.5 ? 'yay!'
    : n < 0.2 ? yield* new FooError({ message: 'Oh no!' })
...
<Aside type="note" title="Initial Execution Included">
  The scheduled recurrences are in addition to the initial execution, so `Micro.repeat(action,
  Micro.scheduleRecurs(1))` executes `action` once initially, and if it succeeds, repeats it an
  additional time.
</Aside>
