...
describe('consumeLanggraphChannels', () => {
  test('routes per-node streams and yields instructor-style snapshots', async () => {
    ...
    const snapshots: Record<string, Array<{ data: unknown; meta: unknown }>> = {}
    const onSnapshot = vi.fn(async (node: string, data: unknown, meta: unknown) => {
      snapshots[node] ??= []
