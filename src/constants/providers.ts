...
import { z } from "zod"
import { thinkingJsonParser, withResponseModel, MODE as ZMODE } from "../core/zod-stream/src"
...
export const PROVIDER_PARAMS_TRANSFORMERS = {
  [PROVIDERS.GROQ]: {
    ...
    >(params: ReturnType<typeof withResponseModel<T, "TOOLS", P>>) {
      if (params.tools && params.tools.some(tool => tool) && params.stream) {
  ...
  [PROVIDERS.ANYSCALE]: {
    ...
    [MODE.TOOLS]: function removeAdditionalPropertiesKeyTools<
      ...
      P extends OpenAI.ChatCompletionCreateParams
    >(params: ReturnType<typeof withResponseModel<T, "TOOLS", P>>) {
      if (params.tools && params.tools.some(tool => tool.function?.parameters)) {
        return {
          ...params,
